...
bool WriteFile(const char *szFile, const string &strData) { return WriteFile(szFile, strData.data(), strData.size()); }
...
bool RemoveFile(const char *szFile) { return (0 == remove(szFile)); }
...
string GetFileSizeString(const char *szFile) { return FormatSize(GetFileSize(szFile), 1024); }
...
uint16_t _Swap(uint16_t value) { return ((value >> 8) & 0x00ff) | ((value << 8) & 0xff00); }
...
uint32_t ByteAlign(uint32_t uValue, uint32_t uAlign) { return (uValue + (uAlign - uValue % uAlign)); }
...
void ZLog::SetLogLever(int nLogLevel) { g_nLogLevel = nLogLevel; }
...
bool ZLog::PrintResult(bool bSuccess, const char *szLog) { return bSuccess ? Success(szLog) : Error(szLog); }
...
bool ZLog::IsDebug() { return (E_DEBUG == g_nLogLevel); }
